function data = sampling_lpvud(TP)
...
if Sy == 1 || Sx == 1
	for i = 1:length(TP.lpv)
		...
		% sampling
		data{i} = sampling_depud(TP.lpv{i}, TP.depidx, TP.domain(TP.depidx,:), gridsize(TP.depidx), TP.siz,TP.X_scaled);
	end
else
	for i = 1:Sy
		for j = 1:Sx
			...
			% sampling
			data{i,j} = sampling_depud(TP.lpv{i,j}, TP.depidx, TP.domain(TP.depidx,:), gridsize(TP.depidx), TP.siz,TP.X_scaled);
		end
	end
end
